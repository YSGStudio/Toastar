...
- **하트 일일 한도**: `artwork_likes` INSERT 전에 Postgres 트리거(`SECURITY DEFINER`)가 좋아요를 누르는 학생이 속한 학급의 `daily_heart_limit`과 그날 사용량을 비교해 초과 시 거부합니다. 한 번 준 하트는 취소할 수 없습니다(RLS에 delete 정책이 없어 API·DB 양쪽에서 막혀 있음). 같은 작품에는 `unique(artwork_id, student_id)` 제약으로 한 번만 줄 수 있습니다.
...
- **작품 설명 욕설·비난 검사**: 작품 업로드 시 "AI의 도움을 받은 점"/"내가 스스로 한 점" 텍스트를 OpenAI(`gpt-4o-mini`)로 검사해 욕설·비속어·타인 비난이 있으면 업로드를 막고 다시 쓰도록 안내합니다(`OPENAI_API_KEY` 필요). API 키가 없거나 호출이 실패하면 업로드 자체를 막지 않도록 통과 처리합니다(외부 API 장애가 핵심 기능을 막지 않게 하기 위함).
